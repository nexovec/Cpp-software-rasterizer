...
{
    GUI *gui = GUI::get();
    gui->add_ui(new GUI_Button({100, 350}, {50, 50}));
    char *text = (char*)"Welcome to my software rasterizer!";
    gui->add_ui(new GUI_Text({70, 250}, 32, text));
    gui->render(back_buffer, asset_cache);
    gui->freeze();
    // gui->cleanup();
}